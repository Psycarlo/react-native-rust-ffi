  ...
  return [
    ...
    // Many params + optional
    {
      ...
      run: async () => {
        const p = createUserProfile(
          ...
          true,
          1000n
        ) as { username: string; followerCount: bigint };
    ...
    },
    {
      ...
      run: async () => {
        const p = createUserProfile(
          ...
          true,
          5000n
        ) as { username: string; displayName: string; location: string };
    ...
    },
    {
      ...
      run: async () => {
        ...
        }) as { users: unknown[]; totalCount: bigint; hasNextPage: boolean };
    ...
    },
    {
      ...
      run: async () => {
        ...
        return `${hex} → [${bytes.length} bytes] → ${back} ${hex === back ? '✓' : '✗'}`;
      },
    },
    ...
    },
    {
      ...
      category: 'Vec Input',
      run: async () => {
        const r = filterStringsByLength(['hi', 'rust', 'ffi', 'bridge', 'ok'], 4);
        return `[${r.join(', ')}]`;
      },
    },
    ...
    // Nullable
    {
      ...
      category: 'Nullable',
      run: async () => {
        const u = findUserByName('alice') as { username: string } | undefined;
    ...
    },
    {
      ...
      run: async () => {
        ...
        return `${u === undefined ? 'undefined ✓' : 'ERROR: expected undefined'}`;
      },
    },
    {
      id: 'async_find_user',
      ...
      category: 'Nullable',
      run: async () => {
        const u = (await asyncFindUserByName('bob', 500n)) as {
          username: string;
        } | undefined;
        return u ? `Found: ${u.username}` : 'undefined';
      },
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerActions}>
          ...
          <Pressable style={[styles.actionBtn, styles.clearBtn]} onPress={clearAll}>
